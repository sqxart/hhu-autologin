...
1. 打开 [Releases 下载页](../../releases)，下载 **`hhu_gui.exe`**（可视化控制台）和 **`hhu_login.exe`**（后台守护），放进同一个文件夹（比如 `D:\校园网工具`）
   ⚠️ 不要点 "Source code"——那只是源代码，不是软件
2. 在浏览器登录一次校园网 → 双击 `hhu_gui.exe` → 点「一键抓取账号和服务」→ 输入密码 → 勾选「开机自启」→ 点「保存配置」
3. 完成！以后开机自动守护，掉线自动恢复（不想用图形界面就看下方命令行方式）
...
- **守护设置**：开机自启开关、检测间隔调节、立即检测登录状态、退出当前校园网账号、断路器提示与清除、打开日志
...
源码运行：`python hhu_gui.py`（同样零依赖，界面用 Python 自带的 tkinter）。
...
- 仅实测 Windows 11 + 常州校区无线网（校园网 / 移动 / 联通服务均验证）；江宁、西康路校区的服务列表由服务器按校区下发，机制上自动适配，欢迎同学验证反馈；macOS / Linux 下核心逻辑通用（`--loop` 模式），安装脚本未适配
