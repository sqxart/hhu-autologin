...
1. 打开 [Releases 下载页](../../releases)，下载 **`hhu-autologin_v1.4.3_portable.zip`**（便携版压缩包），解压后按里面的 **「使用前必读！！！.txt」** 操作
   ⚠️ 不要点 "Source code"——那只是源代码，不是软件
   （也可以只下载 `hhu_gui.exe` + `hhu_login.exe` 两个文件，放进同一个文件夹，比如 `D:\校园网工具`）
2. 将你下载的**`hhu-autologin_v1.4.3_portable.zip`**放在一个固定位置，不要放在桌面也不要留在下载文件夹，放在一个你记得在哪且之后不会再移动/乱动的地方，或者直接去C盘（D盘也行）新建一个文件夹放进去，再解压**`hhu-autologin_v1.4.3_portable.zip`**。
...
> - 想确认它在工作：运行 `hhu_login.exe --check` 看自检结果（会弹出窗口显示报告，看完按任意键关闭），或看 `logs\hhu_login_debug.log` 最后一行的时间

## 特性

- **掉线自动重登**：每分钟检测一次，在线时 0.2 秒静默退出，掉线后 3 秒内自动恢复
- **后台全静默**：守护 exe 按无控制台方式打包，计划任务拉起时不创建任何窗口（v1.4.3 修复定时闪黑框的老问题）；想手动看自检/配置向导，运行 `--check` / `--setup` 照常有输出
...
3. 运行 `hhu_login.exe --check` 看自检结果（v1.4.3 起报告显示在弹出窗口里，看完按任意键关闭）
...
| 后台守护每隔几分钟闪一下黑框 | v1.4.2 及以前的便携版：守护 exe 被打成了控制台程序，计划任务每次唤醒它，Windows 就分一个控制台窗口 | **v1.4.3 起已根治**（守护 exe 改为无控制台打包，实测计划任务上下文零窗口）；旧版本请升级，或临时改用 `install.bat` 的 pythonw 方式注册 |
| 运行 `--check` 后窗口一闪就没了 | 自己开的控制台窗口会随进程退出关闭 | v1.4.3 起 `--check` / `--setup` 会提示"按任意键关闭（60 秒后自动关闭）"；也可 `hhu_login.exe --check > 自检.txt` 重定向到文件慢慢看 |
