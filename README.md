...
1. 打开 [Releases 下载页](../../releases)，下载 **`河海校园网自动登录_v1.4.0_便携版.zip`**，解压后按里面的 **「使用前必读！！！.txt」** 操作
   ⚠️ 不要点 "Source code"——那只是源代码，不是软件
   （也可以只下载 `hhu_gui.exe` + `hhu_login.exe` 两个文件，放进同一个文件夹，比如 `D:\校园网工具`）
2. 将你下载的**`河海校园网自动登录_v1.4.0_便携版.zip`**放在一个固定位置，不要放在桌面也不要留在下载文件夹，放在一个你记得在哪且之后不会再移动/乱动的地方，或者直接去C盘（D盘也行）新建一个文件夹放进去，再解压**`河海校园网自动登录_v1.4.0_便携版.zip`**。
...
> - 文件夹请放在 **D 盘C盘等固定位置**，这个位置应该是固定位置你记得在哪且不会再移动它了，放好之后再，不要放桌面或下载文件夹（计划任务会记住这个位置，移动后需重新设置开机自启）
